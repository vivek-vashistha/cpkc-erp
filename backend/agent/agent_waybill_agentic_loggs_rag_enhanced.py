# agent_waybill_agentic_rag_enhanced.py
# RAG-Enhanced LangGraph agent with verbose, step-by-step logging.
#
# Requirements:
...
def agent_node(state: MessagesState) -> dict:
    ...
    if not messages or not isinstance(messages[0], SystemMessage):
        messages = [SystemMessage(content=RAG_ENHANCED_SYSTEM)] + messages

    # ---- LOG: agent thought / plan before tools ----
    print("\n[RAG-ENHANCED AGENT] ====== Thought / Reply ======")
    if len(messages) > 1 and isinstance(messages[-1], HumanMessage):
        print(f"[RAG-ENHANCED AGENT] User: {messages[-1].content}")
    # invoke
    resp = llm.invoke(messages)
    # print assistant content (before tool calls)
    if isinstance(resp, AIMessage):
        print(f"[RAG-ENHANCED AGENT] Assistant: {resp.content}")
        if hasattr(resp, 'tool_calls') and resp.tool_calls:
            print(f"[RAG-ENHANCED AGENT] Tool calls: {len(resp.tool_calls)}")
        else:
            print("[RAG-ENHANCED AGENT] No tool calls - final response")
...
def tools_node(state: MessagesState) -> dict:
    ...
    for call in last.tool_calls:
        ...
        # LOG: tool call
        print("\n[RAG-ENHANCED TOOL] >>> Call")
        print(f"[RAG-ENHANCED TOOL] name: {name}")
        print(f"[RAG-ENHANCED TOOL] args: {_pretty(args)}")
        ...
        # LOG: tool result
        print("[RAG-ENHANCED TOOL] <<< Result")
...
if __name__ == "__main__":
    ...
    user_msg = HumanMessage(content=f"Check waybill {waybill_id} for anomalies using RAG-enhanced analysis. Use search_anomaly_patterns() and get_learned_insights() to enhance your analysis.")
